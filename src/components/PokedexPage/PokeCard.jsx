...
const PokeCard = ({url}) => {
  ...
  return (
    <article className={`pokecard ${colors}-border`} onClick={handleClick}>
        <header className={`pokecard__header ${colors}-gradient`} >
        ...
        <section className="pokecard__body">
            <h3 className={`pokecard__name ${colors}-color`}>{pokemon?.name}</h3>
            ...
            <ul className="pokecard__stats">
                {
                    pokemon?.stats.map(statInfo => (
                        <li className="pokecard__stat" key={statInfo.stat.url}>
                            ...
                            <span className={`pokecard__stat__value ${colors}-color`}>{statInfo.base_stat}</span>
                        </li>
                    ))
                }
            </ul>
